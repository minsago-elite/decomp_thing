"""Strict GCC oracle build-record and ELF twin verification.
...
def validate_build_record(
    ...
    """Validate a closed, source-bound GCC build record."""
    ...
    if _string(container["platform"], "build record.environment.container.platform") != (
        ...
        raise VerificationError("the GCC oracle build platform must be linux/amd64")
    ...
    if source_directory in build_directory.parents or build_directory in source_directory.parents:
        raise VerificationError("the GCC build must be out of tree, not nested in its source")
...
def _derive_equivalence(full: Mapping[str, Any], stripped: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if full_elf["header"]["class"] != "ELF64" or full_elf["header"]["machine"] != 62:
        raise VerificationError("the GCC oracle pair must be x86-64 ELF64")
    if full_elf["header"]["dataEncoding"] != "little-endian":
        raise VerificationError("the GCC oracle pair must use little-endian ELF encoding")
    if full_elf["header"]["type"] not in {2, 3}:
        raise VerificationError("the GCC oracle driver must be ET_EXEC or ET_DYN")
