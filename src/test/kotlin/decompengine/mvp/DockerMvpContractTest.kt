...
class DockerMvpContractTest {
    ...
    fun `end to end validation script checks pinned input output layout and evidence`() {
        ...
        val cleanup = script.substringAfter("cleanup() {").substringBefore("\n}\ntrap cleanup EXIT")
        assertTrue(cleanup.indexOf("if [[ \"\$binds_ready\" == \"true\"") < cleanup.indexOf("--entrypoint find"))
        val cleanupArmed = script.indexOf("\nbinds_ready=true")
        assertTrue(cleanupArmed > script.indexOf("INPUT_DIR=\"\$validation_root/input\""))
        assertTrue(cleanupArmed > script.indexOf("OUTPUT_DIR=\"\$validation_root/output\""))
        assertTrue(cleanupArmed < script.indexOf("fixture-builder --version"))
