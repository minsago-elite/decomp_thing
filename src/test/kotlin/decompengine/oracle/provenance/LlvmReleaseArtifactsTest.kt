...
class LlvmReleaseArtifactsTest {
    ...
    fun `workflow and legacy surfaces make Kotlin acquisition authority explicit`() {
        ...
        assertTrue(build.contains("decompengine.oracle.provenance.LlvmArtifactManifestVerifierCli"))
        listOf(model, rebuild).forEach { workflow ->
            ...
            assertTrue(workflow.contains("./gradlew --no-daemon verifyLlvmOracleArtifacts"))
