...
`GET /api/v1/jobs/{jobId}/runs/{runId}/cancellation` returns a private `cancellationPolicy` snapshot with the current run, `eligible` boolean and nullable `reasonCode`. Its ETag is the current run version. Reads reject conditional headers and perform no receipt publication, worker signalling, storage recovery or reservation.

| Eligibility result | Meaning for a new command |
| --- | --- |
| `eligible: true`, null reason | The exact queued/running attempt has an owned durable task and space for a new receipt. |
| `ATTEMPT_TERMINAL` | This attempt already has a terminal outcome. A newer attempt is a different target. |
| `CANCELLATION_PENDING` | Cancellation was requested; termination is not confirmed yet. |
| `NO_OWNED_WORKER` | The nonterminal record has no matching task in this service. Recovery must reconcile it. |
| `CANCELLATION_RECEIPT_CAPACITY` | No new command key can be recorded. Existing actor-scoped keys remain replayable. |

Eligibility can change after the read and is not mutation authority. Every PUT rechecks the exact version and command scope. A caller retrying an existing ambiguous intent must preserve that intent's original key/version; the policy describes eligibility for a **new** command. Storage uncertainty or service shutdown returns an error rather than a positive eligibility snapshot. The closed schema rejects contradictory state/reason combinations.
...
Real HTTP tests use an inert registered adapter and an owned queued task through WebApiController. Service tests separately cover running workers, completion races, accepted-reference preservation and restart replay. This adds no production adapter registration, workflow start capability or browser cancellation control. Browser eligibility presentation and end-to-end running-worker HTTP/browser scenarios remain open before completing #485/#180.
