...
The captured full-export assessment now records byte lengths for the input, exporter and Ghidra archive, plus the actual loader language and compiler specification from the completed exporter state. `GccDriverStructuralInputsV1.bindFullExport()` accepts the retained full-export operation and validates its canonical intent, full-recovery runtime selection, input/exporter/archive identities, contained-execution receipt, and linked export-assessment receipt before binding the snapshot to the authenticated stripped GCC driver and target profile. It reparses canonical model bytes, checks the function inventory, and rejects function addresses outside the ELF-derived executable ranges or IDs that do not encode those addresses. The v2 binding includes the receipt-lineage digest alongside the target descriptor, image base and output-tree digest. This is provenance evidence only; it still cannot create `VerifiedStructuralInputsV1` or authorize scoring.

Focused verification covers matching and substituted input, exporter, loader, target, function-address and receipt-lineage cases. It uses a synthetic canonical model and synthetic self-consistent receipts with the checked profile metadata; it does not claim a live GCC driver export, independent identity replay, production score or production qualification.
