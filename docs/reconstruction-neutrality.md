...
scanner without compiling or executing application code. It stays out of Gradle
`check` and `scripts/ci.sh` while the migration leaves known findings; run it
directly (or from `scripts/check-generic-leakage.sh`) instead. Exit status is 0
for no findings, 1 for findings, and 2 for invalid policy or unreadable inputs. JSON output contains either the scan
