...
scanner without compiling or executing application code. Gradle `check` includes
it; `scripts/ci.sh` keeps the gate off the required CI path until the repository
scan passes (tracked by #84). Exit status is 0 for no findings, 1 for findings, and
