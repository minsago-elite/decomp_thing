...
scanner without compiling or executing application code. It is currently an
opt-in draft gate; Gradle `check` and `scripts/ci.sh` do not include it until the
remaining repository findings are resolved. Exit status is 0 for no findings, 1
for findings, and 2 for invalid policy or unreadable inputs. JSON output
