...
| Public DTOs omit private storage/diagnostic data | Satisfied | One job projection excludes persistence paths/messages and computes a public-only v1 version; fixed diagnostic code/message projection rejects raw exception/persisted text; one progress classifier serves legacy and v1 while omitting raw labels, prose, paths and plan entries. Intentionally retained identifiers and non-DTO content endpoints are recorded in `web-public-dto-privacy.md`. |
