...
| Public DTOs omit private storage/diagnostic data | Satisfied | One job projection excludes persistence paths/messages and validates ELF category vocabulary. Legacy/no-attempt v1 versions are public-only hashes; durable versions are validated opaque store tokens to preserve pin/lifecycle CAS. Fixed diagnostic code/message projection rejects raw exception/persisted text; one progress classifier serves legacy and v1 while omitting raw labels, prose, paths and plan entries. Intentionally retained identifiers and non-DTO content endpoints are recorded in `web-public-dto-privacy.md`. |
