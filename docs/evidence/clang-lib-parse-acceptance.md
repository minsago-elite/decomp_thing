# `clang-lib-parse` acceptance evidence

This checkpoint records the authenticated source ownership boundary for issue
#1009. It is evidence for bounded planning and dispatch only; it does not claim
that an implementation has been generated, accepted, merged, or qualified for
production.

## Authenticated population

The LLVM 22.1.6 planning inventory binds `clang-lib-parse` to exactly 18
distinct linked, handwritten compilation units under
`source/clang/lib/Parse/`. The checked planning registry exposes an exact
shard-owner lookup and rejects invalid or unknown shard IDs. The 18-module
planning population is context for dispatch; it is not an emitted-function or
accepted-implementation denominator.

The accompanying control test verifies the exact count, unique source and unit
identities, handwritten classification, module-to-unit identity binding, shard
ownership, source-root boundary, absence of source-only owners, and fail-closed
unknown-shard behavior.
...
retained accepted checkpoint is present in this slice. The 18 linked units
therefore remain unresolved and release-blocking under #1009.
