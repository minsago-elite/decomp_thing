# `clang-lib-driver` acceptance evidence

This checkpoint records the authenticated source ownership boundary for issue
#999. It is evidence for bounded planning and dispatch only; it does not claim
that an implementation has been generated, accepted, merged, or qualified for
production.

## Authenticated population

The LLVM 22.1.6 inventories bind `clang-lib-driver` to exactly 75 linked,
handwritten compilation units under `source/clang/lib/Driver/`. The focused
control test checks every returned module's source prefix and module-to-unit
identity, handwritten classification, the exact owner count, and the one
source-only record:

- `source/clang/lib/Driver/OffloadBundler.cpp`

The planning registry exposes an exact shard-owner lookup. It rejects invalid
and unknown shard IDs, returns only authenticated linked/generated modules, and
does not authorize implementation acceptance or turn module ownership into an
...
retained accepted checkpoint is present in this slice. The 75 linked units
therefore remain unresolved and release-blocking under #999.

Required work still must derive the current emitted population for these exact
owners, generate every required implementation through the qualified bounded
ACP workflow, and retain per-module source plus ACP/validation receipts.
...
The focused planning-control test is rerun with this evidence replacement.
No production reconstruction or qualification run is claimed.
