# `llvm-lib-asmparser` acceptance evidence

This checkpoint records the authenticated source ownership boundary for issue
#1019. It is evidence for bounded planning and dispatch only; it does not claim
that an implementation has been generated, accepted, merged, or qualified for
production.

## Authenticated population

The LLVM 22.1.6 inventories bind `llvm-lib-asmparser` to exactly four linked,
handwritten compilation units:

| Module/unit ID | Source path |
| --- | --- |
| `cu-78eb990ecafba4176c4c017e321dcf47` | `source/llvm/lib/AsmParser/AsmParserContext.cpp` |
| `cu-5ebeb555c99ebf7d0f2a1b23ed4da4dc` | `source/llvm/lib/AsmParser/LLLexer.cpp` |
| `cu-9e37fce1a8fa26f616d665d7f85b1712` | `source/llvm/lib/AsmParser/LLParser.cpp` |
| `cu-b17b6d8176d811e2b042816919674a7c` | `source/llvm/lib/AsmParser/Parser.cpp` |

The authenticated source inventory contains no source-only record for this
shard. These four module rows establish planning ownership context; they are
not an emitted-function or accepted-implementation denominator.

The planning registry exposes an exact shard-owner lookup that rejects invalid
or unknown shard IDs. It does not authorize implementation acceptance or
silently turn module ownership into an emitted-function count.
...
therefore remain unresolved and release-blocking under #1019.
