# `clang-lib-index` acceptance evidence

This checkpoint records the authenticated source ownership boundary for issue
#1005. It is evidence for bounded planning and dispatch only; it does not claim
that an implementation has been generated, accepted, merged, or qualified for
production.

## Authenticated population

The LLVM 22.1.6 inventories bind `clang-lib-index` to exactly one linked,
handwritten compilation unit:

| Module/unit ID | Source path |
| --- | --- |
| `cu-aaeb50637488dcdad9827a10d5763b21` | `source/clang/lib/Index/USRGeneration.cpp` |

The authenticated source inventory contains eight source-only records for this
shard: `CommentToXML.cpp`, `FileIndexRecord.cpp`, `IndexBody.cpp`, `IndexDecl.cpp`,
`IndexSymbol.cpp`, `IndexTypeSourceInfo.cpp`, `IndexingAction.cpp`, and
`IndexingContext.cpp`. The planning registry test verifies the exact linked
owner, module identity, shard and source-root binding, source-only exclusion,
and fail-closed unknown-shard behavior.

The module row establishes planning ownership context; it is not an emitted-
function or accepted-implementation denominator.
...
therefore remains unresolved and release-blocking under #1005.
