...
No HTTP route or browser control is added. HTTP callers must authenticate and authorize before deriving the actor or looking up any receipt. Store/service replay after reopen reuses the same supplied actor identity; a fresh browser session has a different identity and is not automatically entitled to the prior session's receipt. This does not restore an invalidated cookie or claim cross-session receipt access. HTTP/UI integration and qualification remain open.
